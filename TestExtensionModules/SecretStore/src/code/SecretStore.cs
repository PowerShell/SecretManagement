...
namespace Microsoft.PowerShell.SecretStore
{
    ...
    public sealed class SecretStoreExtension : SecretManagementExtension
    {
        ...
        private SecretStoreExtension() : base(string.Empty)
        { }

        public SecretStoreExtension(string vaultName) : base(vaultName)
        { }
        ...
        public override bool SetSecret(
            ...
            object secret,
            string vaultName,
            System.Collections.Generic.IReadOnlyDictionary<string, object> additionalParameters,
            out Exception error)
        {
            error = null;
            if (LocalSecretStore.GetInstance().WriteObject(
                name: name,
            ...
            {
                error = new PSInvalidOperationException(errorMsg);
            }
            ...
            out Exception error)
        {
            ...
            if (LocalSecretStore.GetInstance().ReadObject(
                name: name,
                outObject: out object returnObject,
                out string errorMsg))
            {
                return returnObject;
            }
        ...
        {
            error = null;
            if (LocalSecretStore.GetInstance().DeleteObject(
                name: name,
                out string errorMsg))
            {
                return true;
        ...
        {
            error = null;
            if (LocalSecretStore.GetInstance().EnumerateObjectInfo(
                filter: filter,
                out SecretInformation[] outSecretInfo,
                vaultName: vaultName,
                out string errorMsg))
            {
                return outSecretInfo;
            }
            ...
            out Exception error)
        {
            ...
            try
            {
                LocalSecretStore.GetInstance(password: vaultKey).UnlockLocalStore(password: vaultKey);
                return true;
            }
            catch (Exception ex)
            {
        ...
        {
            // TODO: Test for valid configuration and password.
            errors = null;
            return true;
        }
